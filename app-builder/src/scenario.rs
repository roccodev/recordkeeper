use std::ops::RangeInclusive;

use bdat::RowRef;
use bdat::Table;
use game_data::scenario::ScenarioRange;
use game_data::scenario::ScenarioRanges;

use crate::const_hash;
use crate::BdatRegistry;

pub fn read_scenario_events(bdat: &BdatRegistry) -> ScenarioRanges {
    let menu_scenario = bdat.table(&const_hash!("MNU_saveload_scenario"));
    let cond_list = bdat.table(&const_hash!("FLD_ConditionList"));
    let cond_scenario = bdat.table(&const_hash!("FLD_ConditionScenario"));

    let (ranges_base, ranges_dlc4): (Vec<_>, Vec<_>) = menu_scenario
        .rows()
        .map(|r| menu_scenario.row(r.id()))
        .flat_map(|row| {
            let base_cond = row[const_hash!("ScenarioCond")]
                .as_single()
                .unwrap()
                .to_integer();
            let dlc_cond = row[const_hash!("ScenarioCondDLC4")]
                .as_single()
                .unwrap()
                .to_integer();
            let chapter = row.id();

            [
                cond_list
                    .get_row(base_cond as usize)
                    .map(|r| (false, chapter, r)),
                cond_list
                    .get_row(dlc_cond as usize)
                    .map(|r| (true, chapter, r)),
            ]
            .into_iter()
            .flatten()
        })
        .map(|(dlc, chapter, row)| (dlc, chapter, cond_to_range(cond_scenario, &row)))
        .partition(|(dlc, _, _)| *dlc);
    ScenarioRanges::new(
        ranges_base
            .into_iter()
            .map(|(_, chapter, range)| ScenarioRange {
                chapter: chapter as u32,
                range,
            }),
        ranges_dlc4
            .into_iter()
            .map(|(_, chapter, range)| ScenarioRange {
                chapter: chapter as u32,
                range,
            }),
    )
}

fn cond_to_range(cond_scenario: &Table, cond_row: &RowRef) -> RangeInclusive<u16> {
    let scenario_cond_id = cond_row[const_hash!("Condition")]
        .as_single()
        .unwrap()
        .to_integer();
    let row = cond_scenario.row(scenario_cond_id as usize);
    let min = row[const_hash!("ScenarioMin")]
        .as_single()
        .unwrap()
        .to_integer()
        .try_into()
        .unwrap();
    let max = row[const_hash!("ScenarioMax")]
        .as_single()
        .unwrap()
        .to_integer()
        .try_into()
        .unwrap();
    min..=max
}
