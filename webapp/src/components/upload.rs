...
pub fn UploadButton(props: &UploadButtonProps) -> Html {
    ...
    let on_upload = Callback::from(move |e: Event| {
        ...
        let reader = read_file(input.files(), move |data| save.on_file_upload(data));
...
fn read_file(
    ...
    callback: impl Fn(Result<Vec<u8>, FileReadError>) + 'static,
) -> Option<FileReader> {
    ...
    gloo::file::callbacks::read_as_bytes(&file, callback).into()
}
